...
loadEndicoReports <- function(directory) {
  ...
  byoColNames <- c(
    'invDate', 'invNbr','productDesc','productNbr','qty', 'discountPerCase','color'
  )

  grayColNames <- c(
    'invDate', 'invNbr','productNbr','qty', 'discountPerCase','color'
  )
  ...
  for(f in fileNames) {
    ...
    for(sheet in aeSheetList[2:length(aeSheetList)]) {
      ...
      aeAllowanceSheet = readxl::read_excel(aeFileDir, sheet = sheet) %>%
        ...
        # drop any values that are NA in the first row
        filter(!is.na(.[[1]]))

      if(allowanceColor %in% c('blue','orange','yellow')) {

        aeAllowanceSheet <- aeAllowanceSheet %>%
          dplyr::select(invoiceDate,invoiceNbr,product,productNbr,cases,dfValuePerCase,color) %>%
        ...
        aeAllowanceSheet <- aeAllowanceSheet %>%
          ...
          select(invDate,invNbr,productDesc,productNbr,qty,discountPerCase,color)

      } else if (allowanceColor %in% c('grey')) {
        aeAllowanceSheet <- aeAllowanceSheet %>%
          mutate(discountPerCase = round(amountDue/quantity,2)) %>%
          select(invDate,invNumber,productId,quantity,discountPerCase,color) %>%
